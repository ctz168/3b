#!/bin/bash
# ============================================================================
#  Ollama + Nanbeige4.1-3B-q4_K_M 一键部署脚本 (macOS)
#  支持: macOS 12+ / Apple Silicon (ARM64) & Intel (x86_64)
#  用法: bash install_ollama_macos.sh
# ============================================================================
...
check_macos_version() {
    local major=$(sw_vers -productVersion | cut -d. -f1)
    if [ "$major" -lt 12 ]; then
        warn "建议 macOS 12+，当前版本: $(sw_vers -productVersion)"
    else
...
check_dependencies() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        error "缺少必要依赖: ${missing[*]}，请使用 Homebrew 安装: brew install ${missing[*]}"
    fi

    # 检查/安装 zstd
    if ! command -v zstd &>/dev/null; then
        if command -v brew &>/dev/null; then
            info "通过 Homebrew 安装 zstd..."
            brew install zstd
        else
            error "缺少 zstd，请安装: brew install zstd"
        fi
    fi
...
install_ollama() {
    ...
    if [ -x "$INSTALL_DIR/bin/ollama" ]; then
        local installed_ver=$("$INSTALL_DIR/bin/ollama" --version 2>&1 | grep -oE 'v[0-9.]+' || echo "unknown")
    ...
    local tarball="ollama-darwin.tgz"
...
export DYLD_LIBRARY_PATH="$INSTALL_DIR/lib/ollama:\${DYLD_LIBRARY_PATH:-}"
export OLLAMA_HOST="127.0.0.1:${PORT}"
export OLLAMA_MODELS="$MODEL_DIR"
export OLLAMA_NUM_PARALLEL="1"
ENVEOF

    # 创建 launchd plist (macOS 服务管理)
    local plist_label="com.user.ollama"
    cat > "$HOME/Library/LaunchAgents/${plist_label}.plist" << PLISTEOF
...
<dict>
    ...
    <array>
        <string>$INSTALL_DIR/bin/ollama</string>
        <string>serve</string>
    </array>
    ...
    <dict>
        ...
        <string>$INSTALL_DIR/lib/ollama</string>
    </dict>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    ...
    <string>$INSTALL_DIR/ollama.log</string>
...
start_server() {
    ...
    if [ -f "$HOME/Library/LaunchAgents/${plist_label}.plist" ]; then
        launchctl load "$HOME/Library/LaunchAgents/${plist_label}.plist" 2>/dev/null
        sleep 3
        if curl -s "http://127.0.0.1:${PORT}/api/tags" &>/dev/null; then
            success "Ollama 服务已通过 launchd 启动"
            return 0
        fi
    fi
...
verify() {
    ...
    echo "  5. 停止服务:      launchctl unload ~/Library/LaunchAgents/com.user.ollama.plist"
    echo "=========================================="
}
...
main() {
    echo ""
    echo "=========================================="
    echo "  Ollama + Nanbeige4.1-3B 部署脚本 (macOS)"
    echo "  模型: $MODEL_NAME"
    echo "=========================================="
    echo ""
